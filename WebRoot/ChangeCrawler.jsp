	...
	function add(){ 
      $.ajax({
            ...
            if(data!=""){
                ...
                $("#${requestScope.csdn.csdnId}",parent.document).children().eq(1).html($('#title').val());
                $("#${requestScope.csdn.csdnId}",parent.document).children().eq(2).html($('#author').val());
                $("#${requestScope.csdn.csdnId}",parent.document).children().eq(3).html($('#style').val());
                $("#${requestScope.csdn.csdnId}",parent.document).children().eq(4).html($('#count').val());
                $("#${requestScope.csdn.csdnId}",parent.document).children().eq(5).html($('#introduction').val());
                $("#${requestScope.csdn.csdnId}",parent.document).children().eq(6).children().attr("href",$('#url').val());
                $("#${requestScope.csdn.csdnId}",parent.document).children().eq(6).children().html($('#url').val());
	            alert("修改成功");
...
$('#button').click(function(){
	...
	<body>
		...
		<form id="AddCrawlerform">
			<table class="altrowstable" id="alternateCrawler" width="100%" border="1">
				<tbody id="AddCrawlerjtable" align="center">
					<tr>
						<td>
							名称/标题
						</td>
						<td>
							<input class="easyui-validatebox textbox" value="${requestScope.csdn.title}" data-options="missingMessage:'请输入名称/标题',required:true" id="title" name="title" style="width: 200px"/>
					    </td>
					</tr>
					<tr>
						<td>
